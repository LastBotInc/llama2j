...
*/
public class RunParallelAttention {
...
//            System.arraycopy(s.v, 0, s.l_value_cache, loff + pos * kv_dim, kv_dim);
            ...
            Pointer finalTmp1CU = tmp1CU;
            Pointer finalAttCU = attCU;
            SlicePointer finalL_key_cacheCU = l_key_cacheCU;
            Pointer finalQCU = qCU;
            ContextCUDA finalCuda = cuda;
            Pointer finalXbCU = xbCU;
            SlicePointer finalL_value_cacheCU = l_value_cacheCU;

            cuda.synchronizeStream(0);

//            int threadCount = ContextCUDA.STREAM_COUNT;
            int threadCount = ContextCUDA.STREAM_COUNT;
            while (p.n_heads % threadCount != 0) {
                threadCount--;
            }
            int headsPerStream = p.n_heads / threadCount;
            CountDownLatch latch = new CountDownLatch(threadCount);

            for (int streamId = 0; streamId < ContextCUDA.STREAM_COUNT; streamId++) {
                final int start = streamId * headsPerStream;
                final int end = start + headsPerStream;
...
//                        attentionIndex, keyBase, kv_dim, queryIndex, pos, head_size);
                        finalCuda.attentionLoop.call(finalStreamId, finalQCU, finalL_key_cacheCU.pointer(), finalAttCU,
                                finalTmp1CU, maxIndex, attentionIndex, keyBase, kv_dim, queryIndex, pos, head_size);

                        // exp and sum
                        finalCuda.expSumNormalize.call(finalStreamId, finalAttCU, finalTmp1CU, maxIndex,
                                attentionIndex, pos + 1);

                        // weighted sum of the values, store back into xb
                        int xbIndex = h * head_size;

                        int valueBase = loff + (h / kv_mul) * head_size;
                        finalCuda.accumWeightedValue.call(finalStreamId, finalXbCU, finalAttCU, finalL_value_cacheCU,
                                pos, xbIndex, valueBase, head_size, kv_dim, attentionIndex);
                    }
                    latch.countDown();
                });
            }
